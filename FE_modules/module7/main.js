...
const getUsersByGender = (users, gender) => users.filter(el => el.gender == gender).map(elem => elem.name)
console.log(getUsersByGender(users, 'male')); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]

/*Получить массив только неактивных пользователей (поле isActive).*/
const getInactiveUsers = users => users.filter(el => !el.isActive).map(elem => elem.name)
...
const getTotalBalance = users => users.map(el => el.balance).reduce((acc, value) => acc + value);
console.log(getTotalBalance(users)); // 20916

/*Массив имен всех пользователей у которых есть друг с указанным именем.*/
const getUsersByFriend = (users, name) => users.filter(el => el.friends.includes(name)).map(elem => elem.name);
...
const getUniqueSkills = users => users.map(el => el.skills)
